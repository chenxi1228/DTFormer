...
class NeighborSampler:
    ...
    def __init__(self, adj_list: list, sample_neighbor_strategy: str = 'uniform', time_scaling_factor: float = 0.0, seed: int = None):
        ...
        for node_idx, per_node_neighbors in enumerate(adj_list):
            ...
            self.nodes_snapshots.append(np.array([x[3] for x in sorted_per_node_neighbors]))
    ...
    def find_neighbors_before(self, node_id: int, interact_time: float, return_sampled_probabilities: bool = False):
        ...
        if return_sampled_probabilities:
            return self.nodes_neighbor_ids[node_id][:i], self.nodes_edge_ids[node_id][:i], self.nodes_neighbor_times[node_id][:i], \
                   self.nodes_snapshots[node_id][:i], self.nodes_neighbor_sampled_probabilities[node_id][:i]
        else:
            return self.nodes_neighbor_ids[node_id][:i], self.nodes_edge_ids[node_id][:i], self.nodes_neighbor_times[node_id][:i], None, self.nodes_snapshots[node_id][:i]
    ...
    def get_all_first_hop_neighbors(self, node_ids: np.ndarray, node_interact_times: np.ndarray, snapshots: np.ndarray):
        """
        ...
        nodes_neighbor_ids_list, nodes_edge_ids_list, nodes_neighbor_times_list, nodes_neighbor_snapshots_list = [], [], [], []
        # get the temporal neighbors at the first hop
        for idx, (node_id, node_interact_time, snapshot) in enumerate(zip(node_ids, node_interact_times, snapshots)):
            # find neighbors that interacted with node_id before time node_interact_time
            node_neighbor_ids, node_edge_ids, node_neighbor_times, _, node_neighbor_snapshots = self.find_neighbors_before(node_id=node_id,
            ...
            nodes_neighbor_snapshots_list.append(node_neighbor_snapshots)

        return nodes_neighbor_ids_list, nodes_edge_ids_list, nodes_neighbor_times_list, nodes_neighbor_snapshots_list

    def random_walk_sampler(self, node_ids, node_interact_times, walk_length):
        """
        Performs a random walk for each node in a batch starting from their given interaction times and returns all visited nodes, edges, and times in lists of lists, similar to get_all_first_hop_neighbors function.

        :param node_ids: np.ndarray, array of starting node IDs
        :param node_interact_times: np.ndarray, array of interaction times from which we start the walk for each node
        :param walk_length: int, the number of steps in each random walk
        :param find_neighbors_before: function, a function to find neighbors before a given time for each node
        :param seed: int, seed for the random number generator to ensure reproducibility
        :return: Three lists of lists containing the node IDs, edge IDs, and interaction times for each step in the walk for each node
        """
        np.random.seed(self.seed)

        # Initialize lists to store results for all nodes in the batch
        all_nodes_neighbor_ids_list = []
        all_nodes_edge_ids_list = []
        all_nodes_neighbor_times_list = []

        for node_id, node_interact_time in zip(node_ids, node_interact_times):
            # Initialize lists to store results for the current node
            walk_nodes = []
            walk_edges = []
            walk_times = []

            current_node_id = node_id
            current_time = node_interact_time

            for _ in range(walk_length):
                # Find neighbors before the current interaction time
                neighbors, edge_ids, times, _ = self.find_neighbors_before(node_id=current_node_id,
                                                                           interact_time=current_time,
                                                                           return_sampled_probabilities=False)

                if len(neighbors) == 0:
                    break  # No more neighbors to visit, break the loop

                # Randomly select one of the neighbors
                selected_index = np.random.choice(len(neighbors))
                selected_neighbor = neighbors[selected_index]
                selected_edge_id = edge_ids[selected_index]
                selected_time = times[selected_index]

                # Append the selected neighbor, edge, and time to the lists
                walk_nodes.append(selected_neighbor)
                walk_edges.append(selected_edge_id)
                walk_times.append(selected_time)

                # Update the current node and time for the next iteration
                current_node_id = selected_neighbor
                # current_time = selected_time

            unique_nodes, unique_edges, unique_times = [], [], []

            seen = set()
            for node, edge, time in zip(walk_nodes, walk_edges, walk_times):
                if node not in seen:
            ...
            # Append the lists for the current node to the overall lists
            all_nodes_neighbor_ids_list.append(unique_nodes)
            all_nodes_edge_ids_list.append(unique_edges)
            all_nodes_neighbor_times_list.append(unique_times)

        return all_nodes_neighbor_ids_list, all_nodes_edge_ids_list, all_nodes_neighbor_times_list
...
def get_neighbor_sampler(data: Data, sample_neighbor_strategy: str = 'uniform', time_scaling_factor: float = 0.0, seed: int = None):
    ...
    for src_node_id, dst_node_id, edge_id, node_interact_time, node_snapshot in zip(data.src_node_ids, data.dst_node_ids, data.edge_ids, data.node_interact_times, data.snapshots):
        adj_list[src_node_id].append((dst_node_id, edge_id, node_interact_time, node_snapshot))
        adj_list[dst_node_id].append((src_node_id, edge_id, node_interact_time, node_snapshot))
